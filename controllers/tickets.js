const {
  StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, PermissionFlagsBits,
} = require('discord.js');
...
module.exports = {
  create: async (interaction, client, ticketType) => {
    ...
    const ticketsCount = (await db.all()).filter((o) => o.id.startsWith('ticket_')).length;
  ...
  claim: async (interaction, client) => {
    ...
    const message = interaction.channel.messages.cache.get(await db.get(`ticket_${interaction.channel.id}.messageId`));
    /* eslint-disable */
  ...
  close: async (interaction, client) => {
    ...
    let reason = client.locales.openedTicket.closeReasonAsk.default;
    ...
    if (client.config.tickets.closeTicketCategoryId) {
      // try to get category
      const closeCategory = await interaction.guild.channels.fetch(client.config.tickets.closeTicketCategoryId).catch((e) => {
        // if channel is undefinded - log
        console.log('close category is undefinded', e);
      });
